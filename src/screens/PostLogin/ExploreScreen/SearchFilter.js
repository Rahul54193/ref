...
import {resetserachstate} from '@app/store/exploreProductSlice/exploreProduct.slice';
const sortingItems = [
...
const SearchFilter = ({
  ...
  console.log(props, 'props of search filter');
  const {exploreProduct, getTopNotchWatchSearching} = props;
    ...
    onSubmit: async (val, {setErrors}) => {
      ...
      try {
        ...
        if (
          ...
          setFilterDataForNextPage(props);
          getTopNotchWatchSearching({...props, page: 1}).then(res => {
            setIsFilter(false);
          });
          // try {

          //   const response = await getTopNotchWatchSearching(props);
          ...
          //   setIsFilter(false);
          // }
          // const res = await getTopNotchWatchSearching(props);
          // console.log(res, 'res--<<<<');

          // if (res.payload.data.current_paage < res.payload.data.last_page) {
          //   setLoadMore(true);
          //   setIsFilter(false);
            ...
            <View
              ...
              <SubmitButton
                onPress={() => {
                  ...
                  getTopNotchWatchSearching(props).then(res => {
                    setIsFilter(false);
                  });

                  // getTopNotchWatchSearching().then(res => {
                  //   // setIsFilter(false);
                  // });
                }}
                lable="Reset"
